...
use rodio::{queue, OutputStreamHandle, PlayError, Sample, Source};
...
impl SinkPro {
    ...
    #[inline]
    pub fn try_new(stream: &OutputStreamHandle) -> Result<SinkPro, PlayError> {
        let (sink, queue_rx) = SinkPro::new_idle();
    ...
    /// Builds a new `Sink`.
    #[inline]
    pub fn new_idle() -> (SinkPro, queue::SourcesQueueOutput<f32>) {
    ...
    /// Gets the total duration of current sound
    #[inline]
    pub fn get_current_duration() {}
}

impl Drop for SinkPro {
    #[inline]
    fn drop(&mut self) {
